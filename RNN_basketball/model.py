...
class Model():
  def __init__(self, config):
    ...
    ### Going to attempt making a Sequential NN with the same parameters as below
    model = keras.Sequential()
    rnn_cells = [tf.keras.layers.LSTMCell(hidden_size) for _ in range(num_layers)]
    ...
    self.x = tf.keras.layers.Input(dtype=tf.float32, shape=(batch_size, crd, sl), name='Input_data')
    print(self.x)
    #self.y_ = tf.keras.Input(tf.int64, shape=[batch_size], name='Ground_truth')
    #self.keep_prob = tf.keras.Input("float")

    with tf.name_scope("LSTM") as scope:
      cell = tf.keras.layers.StackedRNNCells([tf.keras.layers.LSTMCell(batch_size) for _ in range(num_layers)])

      inputs = tf.unstack(self.x, axis=2)
      # outputs, _ = tf.nn.rnn(cell, inputs, dtype=tf.float32)
      outputs = tf.keras.layers.RNN(tf.keras.layers.LSTMCell(batch_size))(self.x)
      print(outputs)
