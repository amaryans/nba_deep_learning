...
### Austin M Comment ###
# When we munge the data, it looks like what it is returing is the trajectories on the way down
# This means that we are training the data based on the trajectory on the way down. I would assume that there is a way to look
# at the trajectory on the way up as well, we can test that too
########################

dl.munge_data(11,sl,db, False)

###
# Center the data (Subtracting the center of the basket values it looks like)
###
...
### Neat to see which data we are looking at
plot = 1
if plot:
  dl.plot_traj_2d(25,'at %.0f feet from basket'%db)

### X_train is the trajectory, formatted as four arrays for each trajectory, [[x][y][z][time]]
### y_train is whether the shot went in or not. Just a 1xn boolean array
...
### Having issues with how the model is built because it is currently in tensorflow v1 whereas we should probably use tensorflow v2
### Not entirely sure how to translate what they are doing with the cells and models and whatnot
#model = Model(config)

model = keras.Sequential()
rnn_cells = [tf.keras.layers.LSTMCell(config['hidden_size']) for _ in range(config['num_layers'])]
lstm_layer = layers.StackedRNNCells(rnn_cells)
model.add(layers.RNN(lstm_layer))
model.add(layers.Dense(1))
model.build((batch_size, crd, sl))
model.compile(loss=tf.keras.losses.BinaryCrossentropy(from_logits=True), optimizer=keras.optimizers.Adam(learning_rate=0.005), metrics = ["accuracy"])
model.summary()

model.fit(X_train, y_train, validation_data=(X_val, y_val), batch_size = config['batch_size'], epochs = 250)
