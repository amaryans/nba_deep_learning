...
class Event:
    ...
    def __init__(self, event, gameid, eventnum):
        ...
        ### Going to try to make a csv which can maybe match up the data with the other project
        # This is just a "Moment" object
        if len(self.moments) == 0:
            pass
        else:
            start_moment = self.moments[0]
            player_array = []
            with open('moment_data.csv', 'a') as f:
                writer = csv.writer(f)
                for moment in self.moments:
                    gameid_print = '00' + str(self.gameid) + '_' + str(self.eventnum)
                    row = [moment.game_clock, moment.shot_clock, moment.ball.x, moment.ball.y, moment.ball.radius]
