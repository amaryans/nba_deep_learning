...
class DataLoad():
  ...
  def munge_data(self, height = 11.0, seq_len = 10.0, dist = 3.0,verbose = True):
    ...
    df_arr = df.values#(['x','y','z','game_clock', 'EVENTMSGTYPE', 'rankc'])
    df = None
  ...
  def plot_traj_2d(self,Nplot,extra_title=' '):
    ...
    """
    fig = plt.figure()
    tmp = np.array(self.data['X_train'][:,:,:2], dtype=np.float64)
    data2 = np.linalg.norm(tmp,axis=2)
    data2 = np.dstack((data2,tmp))
    N = data2.shape[0]
    print(self.data)
